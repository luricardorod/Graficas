...
#elif defined(DRONE_CAM_PASS)

float4 FS(VS_OUTPUT input) : SV_TARGET
{
	return TextureRGB.Sample(SS, input.texture0);
}
...
FS_OUT FS( VS_OUTPUT input )
{
	...
	fout.depth		= input.Pos.z / CameraInfo.y; //Lineal
	//fout.depth		= input.Pos.z / input.Pos.w; //No lineal
